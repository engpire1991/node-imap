...
class Connection extends EventEmitter {
  ...
  connect() {
    ...
    const onParserError = function(err){
      // send error if _curReq and its bodyEmitter are set
      if (self._curReq && self._curReq.bodyEmitter){
        self._curReq.bodyEmitter.emit('error', err);
      }
      
      // set _curReq as undefined so that further requests can be made
      self._curReq = undefined;
    }
    this._parser = parser = new Parser(this._sock, this.debug, onParserError)
